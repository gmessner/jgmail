 ...
 */
public class AuthTypeTag extends BodyTagSupport {
    ...
    public final int doAfterBody () throws JspException {
        ...
        if (mt == null) {
            throw new JspException(
				"authType tag must be nested within a mail tag");
        }

        BodyContent body = getBodyContent();
        String authTypeString = body.getString();
        body.clearBody();

        if (authTypeString != null) {

            authTypeString = authTypeString.trim();
            if (authTypeString.length() > 0) {
                mt.setAuthType(AuthTag.getAuthType(authTypeString));
                return SKIP_BODY;
            }
        }

        throw new JspException("authType tag cannot be empty");
    }
}
